...
public class CartController {
    ...
    @PostMapping
    public ResponseEntity<Cart> addToCart(@RequestBody CartItem item, @RequestParam String username) {
        Cart updatedCart = cartService.addToCart(username, item);
        return new ResponseEntity<>(updatedCart, HttpStatus.OK);
    }

    @PutMapping("/{itemId}")
    public ResponseEntity<Cart> updateCartItemQuantity(@PathVariable Long itemId, @RequestParam int quantity, @RequestParam String username) {
        Cart updatedCart = cartService.updateCartItemQuantity(username, itemId, quantity);
        return new ResponseEntity<>(updatedCart, HttpStatus.OK);
    }

    @DeleteMapping("/{itemId}")
    public ResponseEntity<Void> removeFromCart(@PathVariable Long itemId, @RequestParam String username) {
